...
func (m *MockWeatherRepository) ReplaceFutureForLocation(ctx context.Context, locationID int, rows []models.WeatherData) error {
	if m.ReplaceFutureForLocationFn != nil {
		return m.ReplaceFutureForLocationFn(ctx, locationID, rows)
	}
	return nil
}
...
func (m *MockHeatMapRepository) GetHeatMapData(ctx context.Context, startDate, endDate time.Time, bounds *heatmap.GeoBounds, minActivity, limit int, routeTypes []string, lightweight bool, gradeOrders []int) ([]models.HeatMapPoint, error) {
