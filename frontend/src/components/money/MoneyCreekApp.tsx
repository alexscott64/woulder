...
function MoneyCreekWorkspace() {
  ...
  return <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: T.app, color: T.ink, fontFamily: T.font }}>
    ...
    <div style={{ flex: 1, display: 'flex', minHeight: 0, position: 'relative' }}>
      ...
      </> : <ContentView view={view} root={root} trails={cragQuery.data?.trails ?? []} notes={notes} uploads={uploads} trash={trashQuery.data?.items ?? []} canWrite={canWrite} mobile={mobile} openBoulder={openBoulder} selectTrail={selectTrail} onOpenComposer={kind => setComposer({ kind })} onEditNote={note => setComposer({ note })} onDeleteNote={note => deleteNote.mutate(note.id)} onDeleteUpload={upload => deleteUpload.mutate(upload.id)} onRestore={id => restoreArea.mutate(id)} onCreateTrail={trail => createTrail.mutate(trail)} onUpdateTrail={(trail, updates) => updateTrail.mutate({ trail, updates })} />}
    </div>
