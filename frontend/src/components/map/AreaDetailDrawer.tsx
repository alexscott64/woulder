...
import { X, TrendingUp, Users, MapPin, Loader2, ExternalLink, Calendar, ChevronRight, ChevronLeft, MessageSquare, Activity } from 'lucide-react';
...
export function AreaDetailDrawer({ areaId, dateRange, isOpen, onClose }: AreaDetailDrawerProps) {
  ...
  const routeTicks = detail?.recent_ticks?.filter(t => t.mp_route_id === selectedRouteId) || [];
        ...
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-3 sm:p-4 z-10">
          ...
          {view === 'route' && selectedRoute && (
            <div className="mt-2 flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span>·</span>
              <span>{selectedRoute.tick_count} {selectedRoute.tick_count === 1 ? 'tick' : 'ticks'}</span>
            </div>
          )}
        </div>
            ...
            {view === 'area' ? (
              <div className="p-3 sm:p-4 space-y-4 sm:space-y-6">
                ...
                <div className="space-y-2">
                  {selectedTab === 'routes' && detail.top_routes && detail.top_routes.length > 0 && (
                    <>
                  ...
                  {selectedTab === 'ticks' && detail.recent_ticks && detail.recent_ticks.length > 0 && (
                    <>
                        ...
                        >
                          <div className="flex justify-between items-start mb-1">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">
                                {tick.rating} · <span className="font-medium">{tick.user_name}</span>
                              </p>
                            </div>
                  ...
                  {selectedTab === 'comments' && detail.recent_comments && detail.recent_comments.length > 0 && (
                    <>
                ...
                {selectedRoute && (
                  <div className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Total Ticks</div>
...
function TabButton({ 
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 border-b-2 text-sm font-medium transition-colors ${
        active
